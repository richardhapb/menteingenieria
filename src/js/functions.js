    ...
    try {
      for (let i = 0; i < formulario.childNodes.length; i++) {
        for (let j = 0; j < formulario.childNodes[i].childNodes.length; j++) {
          ...
          if (
            !formulario.childNodes[i].isSameNode(
...
function validarFormularioVacio (
  ...
  for (let i = 0; i < elementos.length; i++) {
    if (
      (elementos[i].value === '' && elementos[i].nodeName !== 'SELECT') ||
      (elementos[i].nodeName === 'SELECT' && elementos[i].selectedIndex === 0)
    ) {
